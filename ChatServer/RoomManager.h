...
public:
	...
	void Init(const int beginRoomNumber_, const int maxRoomCount_, const int maxRoomUserCount_)
	{
	...
	UINT16 EnterUser(int roomNumber_, User* user_)
	{
	...
	INT16 LeaveUser(int roomNumber_, User* user_)
	{
		auto pRoom = GetRoomByNumber(roomNumber_);
		if (pRoom == nullptr)
		{
		...
		user_->SetUserState(User::USER_STATE::LOGIN);
		pRoom->LeaveUser(user_);
		return (INT16)ERROR_CODE::NONE;
	}

	Room* GetRoomByNumber(int number_)
	{
